...
bool getConstraints(cinempc::GetUserConstraints::Request &req, cinempc::GetUserConstraints::Response &res)
{
  ...
  Eigen::Matrix<double, 3, 3> wRtarget_perception = cinempc::quatToRMatrix<double>(
      cinempc::calculate_world_pose_from_relative<double>(req.targets_relative.at(0).pose_top, req.drone_pose)
          .orientation);

  Eigen::Matrix<double, 3, 3> dRtarget_perception =
      cinempc::quatToRMatrix<double>(req.targets_relative.at(0).pose_top.orientation);
  ...
  if (sequence == 1)
  {
    ...
    c.targets_im_top_star.at(0).x = image_x_third_right;
    c.weights.w_img_targets.at(0).x = 100;             // 10;                      // 1;                       // 1 * 1;
    c.targets_im_top_star.at(0).y = image_y_third_up;  // mid-body (control with calculations of positions)
    c.weights.w_img_targets.at(0).y_top = 50;          // 10;                      // 1;                       // 1 * 1;
    c.targets_im_bottom_star.at(0).y = image_y_third_down;  // mid-body (control with calculations of positions)
    c.weights.w_img_targets.at(0).y_bottom = 50;            // 1;                       // 1 * 1;
    c.weights.w_img_targets.at(0).y_center = 0;             // 1;                       // 1 * 1;

    c.targets_im_top_star.at(1).x = image_x_third_left;
    c.weights.w_img_targets.at(1).x = 200;             // 10;                      // 1;                       // 1 * 1;
    c.targets_im_top_star.at(1).y = image_y_third_up;  // mid-body (control with calculations of positions)
    c.weights.w_img_targets.at(1).y_top = 0;           // 10;                      // 1;                       // 1 * 1;
    c.targets_im_bottom_star.at(1).y = image_y_third_down;  // mid-body (control with calculations of positions)
    c.weights.w_img_targets.at(1).y_bottom = 0;             // 1;                       // 1 * 1;
    c.targets_im_center_star.at(1).y = image_y_center;      // mid-body (control with calculations of positions)
    c.weights.w_img_targets.at(1).y_center = 50;            // 1;                       // 1 * 1;

    // c.targets_im_top_star.at(1).x = -1;
    // c.weights.w_img_targets.at(1).x = 0;
    // c.targets_im_top_star.at(1).y = -1;  // mid-body (control with calculations of positions)
    // c.weights.w_img_targets.at(1).y = 0;
    // c.targets_im_bottom_star.at(1).y = -1;  // mid-body (control with calculations of positions)
