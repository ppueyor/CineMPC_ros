...
bool getConstraints(cinempc::GetUserConstraints::Request &req, cinempc::GetUserConstraints::Response &res)
{
  ...
  Eigen::Matrix<double, 3, 3> wRboy = cinempc::quatToRMatrix<double>(
      req.targets.at(0).poses_up.at(0).orientation);  // cinempc::RPYtoRMatrix<double>(0, 0, subject_yaw);
  cinempc::RPY<double> RPY_boy = cinempc::quatToRPY<double>(req.targets.at(0).poses_up.at(0).orientation);
  ...
  if (sequence == 1)
  {
    ...
    cinempc::RPY<double> relative =
        cinempc::RMatrixtoRPY<double>(cinempc::RPYtoRMatrix<double>(0, 0, RPY_boy.yaw - PI / 2).transpose() * wRboy);
