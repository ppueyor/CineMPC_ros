/**
 * Implementation of KalmanFilter class.
 *
 * @author: Hayk Martirosyan
 * @date: 2014.11.15
 */

#include <iostream>
#include <stdexcept>

#include "cinempc/cinempc_kalman.hpp"

KalmanFilterEigen::KalmanFilterEigen(double dt, const Eigen::MatrixXd& A, const Eigen::MatrixXd& C,
                                     const Eigen::MatrixXd& Q, const Eigen::MatrixXd& R, const Eigen::MatrixXd& P,
                                     int number_states, int number_measurements)
  : A(A), C(C), Q(Q), R(R), P0(P), m(C.rows()), n(A.rows()), dt(dt), initialized(false), I(n, n), x_hat(n), x_hat_new(n)
{
  I.setIdentity();
  n = number_states;
  m = number_measurements;
}

KalmanFilterEigen::KalmanFilterEigen()
{
}

int KalmanFilterEigen::numberOfStates()
{
  return n;
}

int KalmanFilterEigen::numberOfMeasurements()
{
  return m;
}

void KalmanFilterEigen::init(double t0, const Eigen::VectorXd& x0)
{
  x_hat = x0;
  P = P0;
  this->t0 = t0;
  t = t0;
  initialized = true;
}

void KalmanFilterEigen::init()
{
  x_hat.setZero();
  P = P0;
  t0 = 0;
  t = t0;
  initialized = true;
}

void KalmanFilterEigen::update(const Eigen::VectorXd& y, bool measure)
{
  if (!initialized)
    throw std::runtime_error("Filter is not initialized!");

  x_hat_new = A * x_hat;
  P = A * P * A.transpose() + Q;
  if (measure)
  {
    K = P * C.transpose() * (C * P * C.transpose() + R).inverse();
    x_hat_new += K * (y - C * x_hat_new);
    P = (I - K * C) * P;
  }
  x_hat = x_hat_new;

  t += dt;
}

Eigen::MatrixXd KalmanFilterEigen::predict(int kf_time_each_mpc, int max_time_steps)
{
  // std::cout << "CALLED   " << kf_time_each_mpc << "   " << max_time_steps << std::endl;
  if (!initialized)
    throw std::runtime_error("Filter is not initialized!");

  Eigen::VectorXd state_n = x_hat;
  Eigen::MatrixXd states_n(max_time_steps, n);

  int max_interval = max_time_steps * kf_time_each_mpc;
  int matrix_index = 0;

  for (int i = 0; i < max_interval; i++)
  {
    state_n = A * state_n;
    if (i % kf_time_each_mpc == 0)
    {
      states_n.row(matrix_index) = state_n;
      matrix_index++;
    }
    // std::cout << "State_n" << state_n << std::endl;
  }
  // std::cout << "States_n" << states_n << std::endl;
  return states_n;
}

void KalmanFilterEigen::update(const Eigen::VectorXd& y, const Eigen::MatrixXd A)
{
  this->A = A;
  update(y, true);
}

void KalmanFilterEigen::update(const Eigen::MatrixXd A)
{
  Eigen::VectorXd empty;
  this->A = A;
  update(empty, false);
}