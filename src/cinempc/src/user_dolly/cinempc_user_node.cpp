...
bool getConstraints(cinempc::GetUserConstraints::Request &req, cinempc::GetUserConstraints::Response &res)
{
  ...
  Eigen::Matrix<double, 3, 3> wRtarget_perception = cinempc::quat_to_R_matrix<double>(req.world_rotations_target.at(0));
  Eigen::Matrix<double, 3, 3> wRtarget_gt = cinempc::RPY_to_R_matrix<double>(0, 0, target_1_yaw_gt);
  ...
  if (sequence == 1)
  {
    ...
    c.weights.w_img_targets.at(0).x = 0.5;
    c.targets_im_top_star.at(0).y = image_y_third_up;
    c.weights.w_img_targets.at(0).y_top = 0.5;
    c.targets_im_bottom_star.at(0).y = -1;
    c.weights.w_img_targets.at(0).y_bottom = 0;
    ...
    cinempc::RPY<double> relative = cinempc::R_matrix_to_RPY<double>(
        cinempc::RPY_to_R_matrix<double>(RPY_target.roll, RPY_target.pitch, RPY_target.yaw - PI).transpose() *
        wRtarget);
  ...
  else if (sequence == 2)
  {
    ...
    c.weights.w_img_targets.at(0).x = 0.5;
    c.targets_im_top_star.at(0).y = image_y_third_up;
    c.weights.w_img_targets.at(0).y_top = 0.75;
    c.targets_im_bottom_star.at(0).y = -1;
    c.weights.w_img_targets.at(0).y_bottom = 0;
    c.targets_im_center_star.at(0).y = image_y_third_down;
    c.weights.w_img_targets.at(0).y_center = 0.75;
