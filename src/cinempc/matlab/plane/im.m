...
plot1 = plot(Time(idgirl) / 1000 ,im_u_d(idgirl),'*', 'LineWidth',12, 'Markersize',12, 'MarkerIndices',1:4:idgirl(end),'Color','#75B2FF'); %light blue
%ylim([0 700]);
hold on
ax = gca;
ax.FontSize = 70; 
plot2 = plot(Time(idgirl) / 1000,im_u(idgirl),'o-', 'LineWidth',8, 'Markersize',15, 'MarkerIndices',1:5:idgirl(end),'Color','#006AAF'); % dark blue

plot3= plot(Time(idgirl) / 1000, im_v_up_d(idgirl),'*', 'LineWidth',12, 'Markersize',12, 'MarkerIndices',1:4:idgirl(end),'Color','#FFC075'); %light orange
 plot(Time(idgirl) / 1000, im_v_down_d(idgirl),'*', 'LineWidth',12, 'Markersize',12, 'MarkerIndices',1:4:idgirl(end),'Color','#FFC075'); 

plot4 =plot(Time(idgirl) / 1000, im_v_up(idgirl),'bo-', 'LineWidth',8, 'Markersize',15, 'MarkerIndices',1:5:idgirl(end),'Color','#FF6442');%dark orange
plot(Time(idgirl) / 1000, im_v_down(idgirl),'bo-', 'LineWidth',8, 'Markersize',15, 'MarkerIndices',1:5:idgirl(end),'Color','#FF6442');
xlim([0 60]);
lgd = legend([plot2, plot1, plot4, plot3],{'im_{t.x}', 'im_{t.x}^*', 'im_{t.y}', 'im_{t.y}^*'},'Orientation','Horizontal');
lgd.FontSize = 70;
