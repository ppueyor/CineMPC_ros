...
template <typename T>
cinempc::IntrinsicsCamera getInstrinscsMsg(T focal_length_in, T focus_distance_in, T aperture_in)
{
  cinempc::IntrinsicsCamera msg;
...
std::string plot_values(cinempc::PlotValues plot_values, bool header)
{
  ...
  if (header)
  {
    ...
    {
      result << "world_x_" << i << "_gt"
             << ","
             << "world_y_" << i << "_gt"
             << ","
             << "world_z_" << i << "_gt"
             << ","
             << "world_x_" << i << "_perception"
             << ","
             << "world_y_" << i << "_perception"
             << ","
             << "world_z_" << i << "_perception"
             << ","
             << "world_x_" << i << "_kf"
             << ","
             << "world_y_" << i << "_kf"
  ...
  else
  {
    try
    {
      ...
      for (int i = 0; i < targets_classes.size(); i++)
      {
        result << plot_values.targets_world_gt[i].x << "," << plot_values.targets_world_gt[i].y << ","
               << plot_values.targets_world_gt[i].z << "," << plot_values.targets_world_perception[i].x << ","
               << plot_values.targets_world_perception[i].y << "," << plot_values.targets_world_perception[i].z << ","
               << plot_values.targets_world_kf[i].x << "," << plot_values.targets_world_kf[i].y << ","
               << plot_values.targets_world_kf[i].z << "," << plot_values.mpc_plot_values.im_u[i] << ","
               << plot_values.mpc_plot_values.im_v_up[i] << "," << plot_values.mpc_plot_values.im_v_center[i] << ","
               << plot_values.mpc_plot_values.im_v_down[i] << "," << plot_values.constraints.targets_im_top_star.at(i).x
               << "," << plot_values.constraints.targets_im_top_star.at(i).y << ","
               << plot_values.constraints.targets_im_center_star.at(i).y << ","
               << plot_values.constraints.targets_im_bottom_star.at(i).y << ",";
      }

      result << plot_values.mpc_plot_values.d_gt << "," << plot_values.v_target_kf.x << "," << plot_values.v_target_kf.y
             << "," << plot_values.v_target_kf.z << "," << plot_values.v_target_gt.x << "," << plot_values.v_target_gt.y
             << "," << plot_values.v_target_gt.z << "," << cinempc::quat_to_RPY<double>(plot_values.target_rot_gt).pitch
             ...
             << plot_values.mpc_plot_values.intrinsics_camera.aperture << "," << plot_values.zoom_steps << ","
             << plot_values.focus_steps << "," << plot_values.mpc_plot_values.dn << ","
             << plot_values.mpc_plot_values.df << "," << plot_values.mpc_plot_values.cost << ","
             ...
             << 0 << "," << 0 << "," << 0 << "," << plot_values.constraints.targets_d_star.at(0) << ","
             << plot_values.drone_position_gt.x << "," << plot_values.drone_position_gt.y << ","
             << plot_values.drone_position_gt.z << "," << cinempc::quat_to_RPY<double>(plot_values.drone_rot_gt).roll
             << "," << cinempc::quat_to_RPY<double>(plot_values.drone_rot_gt).pitch << ","
             << cinempc::quat_to_RPY<double>(plot_values.drone_rot_gt).yaw << std::endl;
    }
    catch (int n)
    {
      std::cout << "Exc ignored" << std::endl;
    }
  }
  return result.str();
}
