                                                         ...
                                                         geometry_msgs::Pose drone_pose)
{
  ...
  Eigen::Matrix<T, 3, 3> wRd = quatToRMatrix<T>(drone_pose.orientation, false);
                                                  ...
                                                  bool from_zero)
{
  ...
  Eigen::Matrix<T, 3, 3> wRd = quatToRMatrix<T>(drone_pose.orientation, false);
  Eigen::Matrix<T, 3, 3> wRs = wRd * quatToRMatrix<T>(drone_steps_pose.orientation, false);
...
template <typename T>
geometry_msgs::Point readPositionImageToWorld(T focal_length_mm, T u_px, T v_px, T z_mm, geometry_msgs::Quaternion q)
{
  ...
  T dx_mm = (z_mm) * (u_mm - x0_mm) / focal_length_mm;

  T dy_mm = (z_mm) * (v_mm - y0_mm) / focal_length_mm;
  ...
  rPs_point.x = dx_m;
  rPs_point.y = dy_m;
  rPs_point.z = z_mm / 1000;

  Eigen::Matrix<T, 3, 1> rPs(rPs_point.z, rPs_point.x, rPs_point.y);

  Eigen::Matrix<T, 3, 3> R = quatToRMatrix<T>(q, false);

  Eigen::Matrix<T, 3, 1> P = R.transpose().inverse() * rPs;
