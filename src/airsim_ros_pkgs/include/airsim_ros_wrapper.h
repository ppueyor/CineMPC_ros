...
private:
  ...
  void set_intrinsics_cb(const airsim_ros_pkgs::IntrinsicsCamera::ConstPtr &intrinsics_cmd_msg,
                         const std::string &vehicle_name,
  ...
  // state, returns the simulation timestamp best guess based on drone state timestamp, airsim needs to return timestap
  // for environment
  ...
  bool land_srv_cb(airsim_ros_pkgs::Land::Request &request, airsim_ros_pkgs::Land::Response &response,
                   const std::string &vehicle_name);
  ...
  /// ROS tf broadcasters
  void publish_camera_tf(const ImageResponse &img_response, const ros::Time &ros_time, const std::string &frame_id,
  ...
  sensor_msgs::ImagePtr get_img_msg_from_response(const ImageResponse &img_response, const ros::Time curr_ros_time,
                                                  const std::string frame_id);
  sensor_msgs::ImagePtr get_depth_img_msg_from_response(const ImageResponse &img_response,
                                                        const ros::Time curr_ros_time, const std::string frame_id);

  void process_and_publish_img_response(const std::vector<ImageResponse> &img_response_vec, const int img_response_idx,
                                        const std::string &vehicle_name);
...
private:
  ...
  // todo - subscriber / services for a GROUP of robots, which is defined by a list of `vehicle_name`s passed in the ros
  // msg / srv request
  ...
  std::unordered_map<std::string, std::unique_ptr<TargetROS>> target_name_ptr_map_;
  ...
  // todo not sure if async spinners shuold be inside this class, or should be instantiated in airsim_node.cpp, and cb
  // queues should be public todo for multiple drones with multiple sensors, this won't scale. make it a part of
  // VehicleROS?
