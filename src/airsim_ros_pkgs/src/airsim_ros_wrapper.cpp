...
const std::unordered_map<int, std::string> AirsimROSWrapper::image_type_int_to_string_map_ = { { 0, "Scene" },
                                                                                               { 1, "DepthPlanar" },
                                                                                               { 2, "DepthPerspectiv"
                                                                                                    "e" },
                                                                                               { 3, "DepthVis" },
...
AirsimROSWrapper::AirsimROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private,
  ...
  ,  // a thread for image callbacks to be 'spun' by img_async_spinner_
...
void AirsimROSWrapper::initialize_airsim()
{
  ...
  try
  {
    ...
    {
      airsim_client_ =
          std::unique_ptr<msr::airlib::RpcLibClientBase>(new msr::airlib::MultirotorRpcLibClient(host_ip_));
    }
    else
    {
    ...
    // todo there's only one global origin geopoint for environment. but airsim API accept a parameter vehicle_name?
    // inside carsimpawnapi.cpp, there's a geopoint being assigned in the constructor. by?
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
  ...
  gimbal_angle_quat_cmd_sub_ =
      nh_private_.subscribe("gimbal_angle_quat_cmd", 50, &AirsimROSWrapper::gimbal_angle_quat_cmd_cb, this);
  gimbal_angle_euler_cmd_sub_ =
      nh_private_.subscribe("gimbal_angle_euler_cmd", 50, &AirsimROSWrapper::gimbal_angle_euler_cmd_cb, this);
  origin_geo_point_pub_ = nh_private_.advertise<airsim_ros_pkgs::GPSYaw>("origin_geo_point", 10);

  airsim_img_request_vehicle_name_pair_vec_.clear();
  target_name_ptr_map_.clear();
  ...
  for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
  {
    ...
    for (auto& curr_camera_elem : vehicle_setting->cameras)
    {
      ...
      vehicle_ros->enable_manual_focus_srv_vec.push_back(
          nh_private_.advertiseService<airsim_ros_pkgs::EnableManualFocus::Request,
              ...
              curr_vehicle_name + "/" + curr_camera_name + "/" + "enable_manual_focus",
      ...
      for (const auto& curr_capture_elem : camera_setting.capture_settings)
      {
        ...
        if (!(std::isnan(capture_setting.fov_degrees)))
        {
          ...
          image_pub_vec_.push_back(
              image_transporter.advertise(curr_vehicle_name + "/" + curr_camera_name + "/" +
        ...
        switch (sensor_setting->sensor_type)
        {
          case SensorBase::SensorType::Barometer:
          {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<airsim_ros_pkgs::Altimeter>(curr_vehicle_name + "/altimeter/" + sensor_name, 10);
            break;
          }
          ...
          case SensorBase::SensorType::Gps:
          {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<sensor_msgs::NavSatFix>(curr_vehicle_name + "/gps/" + sensor_name, 10);
            break;
          }
          ...
          case SensorBase::SensorType::Distance:
          {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<sensor_msgs::Range>(curr_vehicle_name + "/distance/" + sensor_name, 10);
            break;
          }
    ...
    auto isLidar = std::function<bool(const SensorPublisher& pub)>(
        [](const SensorPublisher& pub) { return pub.sensor_type == SensorBase::SensorType::Lidar; });
    ...
    vehicle_name_ptr_map_.emplace(curr_vehicle_name, std::move(vehicle_ros));  // allows fast lookup in command
  ...
  if (vehicle_name_ptr_map_.size() > 1 && airsim_mode_ == AIRSIM_MODE::DRONE)
  {
    ...
    vel_cmd_all_body_frame_sub_ =
        nh_private_.subscribe("all_robots/vel_cmd_body_frame", 1, &AirsimROSWrapper::vel_cmd_all_body_frame_cb, this);
    vel_cmd_all_world_frame_sub_ =
        nh_private_.subscribe("all_robots/vel_cmd_world_frame", 1, &AirsimROSWrapper::vel_cmd_all_world_frame_cb, this);
    ...
    takeoff_group_srvr_ =
        nh_private_.advertiseService("group_of_robots/takeoff", &AirsimROSWrapper::takeoff_group_srv_cb, this);
  ...
  // targets state
  string targets[] = { "Person1", "Person2" };
...
bool AirsimROSWrapper::enable_manual_focus_srv_cb(airsim_ros_pkgs::EnableManualFocus::Request& request,
  ...
  if (isENU_)
  {
    try
    {
      ...
      auto transformStampedENU =
          tf_buffer_.lookupTransform(AIRSIM_FRAME_ID, vehicle_name, ros::Time(0), ros::Duration(1));
  ...
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
  {
    ...
    for (auto& sensor_publisher : vehicle_ros->sensor_pubs)
    {
      switch (sensor_publisher.sensor_type)
      {
        ...
        {
          auto distance_data =
              airsim_client_->getDistanceSensorData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
    ...
    {
      airsim_ros_pkgs::IntrinsicsCamera intrinsics_msg;
      intrinsics_msg.aperture =
          airsim_client_->simGetFocusAperture(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
      intrinsics_msg.focal_length =
          airsim_client_->simGetFocalLength(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
      intrinsics_msg.focus_distance =
          airsim_client_->simGetFocusDistance(intrinsics_publisher.camera_name, vehicle_ros->vehicle_name, false);
...
void AirsimROSWrapper::publish_target_state()
{
  for (auto& target_name_ptr_pair : target_name_ptr_map_)
  {
    ...
    object_state_msg.pose.position.x = p.position.x();
    object_state_msg.pose.position.y = p.position.y();
    object_state_msg.pose.position.z = p.position.z();
    object_state_msg.pose.orientation.x = p.orientation.x();
    object_state_msg.pose.orientation.y = p.orientation.y();
    object_state_msg.pose.orientation.z = p.orientation.z();
  ...
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
  {
    ...
    if (airsim_mode_ == AIRSIM_MODE::DRONE)
    {
      ...
      if (drone->has_vel_cmd)
      {
        std::lock_guard<std::mutex> guard(drone_control_mutex_);
        static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())
            ->moveByVelocityAsync(drone->vel_cmd.x, drone->vel_cmd.y, drone->vel_cmd.z, vel_cmd_duration_,
...
void AirsimROSWrapper::append_static_vehicle_tf(VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting)
{
  ...
  if (isENU_)
  {
    std::swap(vehicle_tf_msg.transform.translation.x,
              vehicle_tf_msg.transform.translation.y);  // @suppress("Invalid arguments")
    std::swap(vehicle_tf_msg.transform.rotation.x,
              vehicle_tf_msg.transform.rotation.y);  // @suppress("Invalid arguments")
                                              ...
                                              const msr::airlib::LidarSimpleParams& lidar_setting)
{
  ...
  if (isENU_)
  {
    std::swap(lidar_tf_msg.transform.translation.x,
              lidar_tf_msg.transform.translation.y);                                  // @suppress("Invalid arguments")
                                               ...
                                               const CameraSetting& camera_setting)
{
  ...
  if (isENU_)
  {
    std::swap(static_cam_tf_body_msg.transform.translation.x,
              static_cam_tf_body_msg.transform.translation.y);  // @suppress("Invalid arguments")
    std::swap(static_cam_tf_body_msg.transform.rotation.x,
              static_cam_tf_body_msg.transform.rotation.y);  // @suppress("Invalid arguments")
                                                            ...
                                                            const CaptureSetting& capture_setting) const
{
  ...
  // todo focal length in Y direction should be same as X it seems. this can change in future a scene capture component
  // which exactly correponds to a cine camera float f_y = (capture_setting.height / 2.0) /
  ...
  for (const auto& curr_img_response : img_response_vec)
  {
    ...
    if (curr_img_response.pixels_as_float)
    {
      image_pub_vec_[img_response_idx_internal].publish(
          get_depth_img_msg_from_response(curr_img_response,  // @suppress("Invalid arguments")
                                          curr_ros_time, curr_img_response.camera_name + "_optical"));
    }
    // Scene / Segmentation / SurfaceNormals / Infrared
    else
    {
      image_pub_vec_[img_response_idx_internal].publish(
          get_img_msg_from_response(curr_img_response,  // @suppress("Invalid arguments")
                                    curr_ros_time, curr_img_response.camera_name + "_optical"));
    }
                                         ...
                                         const std::string& frame_id, const std::string& child_frame_id)
{
  ...
  if (isENU_)
  {
    std::swap(cam_tf_body_msg.transform.translation.x,
              cam_tf_body_msg.transform.translation.y);  // @suppress("Invalid arguments")
    std::swap(cam_tf_body_msg.transform.rotation.x,
              cam_tf_body_msg.transform.rotation.y);  // @suppress("Invalid arguments")
